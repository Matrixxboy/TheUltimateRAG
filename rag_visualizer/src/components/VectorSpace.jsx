...
const VectorSpace = ({ data }) => {
    ...
    return (
        <div className="glass-panel p-6 rounded-2xl h-[400px] flex flex-col border-l-4 border-l-blue-500">
            <h2 className="text-xl font-bold mb-4 text-slate-800 flex justify-between items-center">
                Vector Space Visualization
                {chartData.length > 0 && <span className="text-xs font-medium text-purple-700 bg-purple-100 px-2 py-1 rounded-full border border-purple-200">PCA Reduced (3D → 2D)</span>}
            </h2>
                ...
                {chartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                            ...
                            <XAxis type="number" dataKey="x" name="PC1" stroke="#94a3b8" tick={{ fontSize: 10 }} tickLine={false} axisLine={{ stroke: '#e2e8f0' }} />
                            <YAxis type="number" dataKey="y" name="PC2" stroke="#94a3b8" tick={{ fontSize: 10 }} tickLine={false} axisLine={{ stroke: '#e2e8f0' }} />
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 bg-slate-50/50 rounded-xl">
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white/95 backdrop-blur border border-slate-200 p-3 rounded-xl shadow-xl max-w-xs z-50 text-slate-800">
                <p className={`font-semibold mb-1 ${data.type === 'query' ? 'text-pink-600' : 'text-blue-600'}`}>
